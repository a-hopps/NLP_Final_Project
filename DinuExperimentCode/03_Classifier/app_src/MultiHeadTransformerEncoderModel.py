...
class MultiHeadTransformerEncoderModel(K.Model):
    def __init__(self, transformer_model, num_classes=5, number_of_difficulty_tags=10, **kwargs):
        ...
        self.tags_classifier = Dense(num_classes, activation='sigmoid', name='tags_classifier')
        # head 2: multiclass (difficulty)
        self.diff_classifier = Dense(number_of_difficulty_tags, activation='softmax', name='diff_classifier')
    ...
    def call(self, inputs, **kwargs):
        ...
        # and return the outputs for both heads
        # shape: (batch_size, num_classes)
        
        return {
            "tags":      self.tags_classifier(linear_output),
            "difficulty": self.diff_classifier(linear_output)
        }
    ...
    def compile_model(
        ...
        learning_rate=2e-5, 
        weight_decay=1e-4, 
        tag_loss_weight: float = 0.5,
        diff_loss_weight: float = 0.5,
        total_steps=None
    ):
        ...
        losses = {
            "tags":      BinaryCrossentropy(name="loss_tags"),
            "difficulty": SparseCategoricalCrossentropy (name="loss_diff"),
        }
        loss_weights = {
            "tags": tag_loss_weight,
            "difficulty": diff_loss_weight,
        }
        ...
        if self.num_classes > 2:
            metrics = {
                ...
                "difficulty": [
                    SparseCategoricalAccuracy(name="acc_diff")
                ],
            }
        else:
            metrics = [
