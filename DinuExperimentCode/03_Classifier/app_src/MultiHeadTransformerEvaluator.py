...
from app_src.MultiHeadTransformerWrapper import MultiHeadTransformerWrapper
...
class MultiHeadTransformerEvaluator():
    ...
    def __save_metrics(self, encoder, estimator_name, metrics_results, number_of_tags):
        ...
        if not os.path.isfile(CONFIG[f'TOP_{number_of_tags}_MULTIHEADDIFF_BENCHMARK_TRANSFORMER_MODELS_PATH']):
            # Write the DataFrame to the csv file
            df.to_csv(CONFIG[f'TOP_{number_of_tags}_MULTIHEADDIFF_BENCHMARK_TRANSFORMER_MODELS_PATH'], index=False)
        else:
            # Append the DataFrame to an existing CSV file
            df.to_csv(CONFIG[f'TOP_{number_of_tags}_MULTIHEADDIFF_BENCHMARK_TRANSFORMER_MODELS_PATH'], index=False, mode='a', header=False)

    def evaluate_model(self, epochs, batch_size, number_of_tags=5, number_of_difficulty_tags=10, model=None, transformer_model_path=None, train_dataset_path=None, val_dataset_path=None, test_dataset_path=None):
        if model is None:
            raise Exception("Model name is required for evaluation")
        else:
            print(f"Training and evaluating model: {model}")
            transformer_wrapper = MultiHeadTransformerWrapper(model, number_of_tags, number_of_difficulty_tags)
